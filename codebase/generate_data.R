# Generate synthetic data (from uniform distribution between 0 and 1) for every independent variable 
# present in <x_vector>. It returns 

generate_data <- function(x_vector, b_vector, values, sd, output_dir){
    # Store generated x values here
    x_values = list()

    # Need a X matrix. First column is 1's
    X = matrix(rep(1, values), ncol = 1)
